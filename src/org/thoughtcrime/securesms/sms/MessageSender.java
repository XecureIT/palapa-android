...
public class MessageSender {
  ...
  public static long send(final Context context,
                          ...
                          final SmsDatabase.InsertListener insertListener)
  {
    return send(context, message, threadId, forceSms, insertListener, null);
  }
  ...
  public static long send(final Context context,
                          ...
                          final boolean forceSms,
                          final SmsDatabase.InsertListener insertListener,
                          final List<RecipientId> xMembers)
  {
    try {
      ...
      sendMediaMessage(context, recipient, forceSms, messageId, message.getExpiresIn(), xMembers);
    ...
    try {
      ...
      for (int i = 0; i < messageIds.size(); i++) {
        ...
        if (isLocalSelfSend(context, recipient, false)) {
          ...
          messageJobs.add(new PushGroupSendJob(messageId, recipient.getId(), null, null));
        } else {
  ...
  public static void resendGroupMessage(Context context, MessageRecord messageRecord, RecipientId filterRecipientId) {
    ...
    sendGroupPush(context, messageRecord.getRecipient(), messageRecord.getId(), filterRecipientId, null);
  }
  ...
  public static void resend(Context context, MessageRecord messageRecord) {
    ...
    if (messageRecord.isMms()) {
      sendMediaMessage(context, recipient, forceSms, messageId, expiresIn, null);
    } else {
      sendTextMessage(context, recipient, forceSms, keyExchange, messageId);
    }
  }

  private static void sendMediaMessage(Context context, Recipient recipient, boolean forceSms, long messageId, long expiresIn, List<RecipientId> xMembers)
  {
    if (isLocalSelfSend(context, recipient, forceSms)) {
      sendLocalMediaSelf(context, messageId);
    } else if (isGroupPushSend(recipient)) {
      sendGroupPush(context, recipient, messageId, null, xMembers);
  ...
  private static void sendGroupPush(Context context, Recipient recipient, long messageId, RecipientId filterRecipientId, List<RecipientId> xMembers) {
    JobManager jobManager = ApplicationDependencies.getJobManager();
    PushGroupSendJob.enqueue(context, jobManager, messageId, recipient.getId(), filterRecipientId, xMembers);
  }
