...
public class GroupMessageProcessor {
                                                  ...
                                                  boolean outgoing)
  {
    ...
    List<RecipientId>       admins  = new LinkedList<>();
    ...
    if (group.getAdmins().isPresent()) {
      for (SignalServiceAddress admin : group.getAdmins().get()) {
        admins.add(Recipient.externalPush(context, admin).getId());
      }
    }

    database.create(id, group.getName().orNull(), members, group.getOwner().orNull(), admins,
                                                  ...
                                                  boolean outgoing)
  {
    ...
    Set<RecipientId> recordAdmins  = new HashSet<>(groupRecord.getAdmins());
    Set<RecipientId> messageAdmins = new HashSet<>();

    if (group.getAdmins().isPresent()) {
      for (SignalServiceAddress messageAdmin : group.getAdmins().get()) {
        messageAdmins.add(Recipient.externalPush(context, messageAdmin).getId());
      }
    }

    Set<RecipientId> addedAdmins = new HashSet<>(messageAdmins);
    addedAdmins.removeAll(recordAdmins);

    Set<RecipientId> missingAdmins = new HashSet<>(recordAdmins);
    missingAdmins.removeAll(messageAdmins);

    GroupContext.Builder builder = createGroupContext(group);
    builder.setType(GroupContext.Type.UPDATE);

    Set<RecipientId> unionMembers = new HashSet<>(recordMembers);
    unionMembers.addAll(addedMembers);
    unionMembers.removeAll(missingMembers);
    ...
    Set<RecipientId> unionAdmins = new HashSet<>(recordAdmins);
    unionAdmins.addAll(addedAdmins);
    unionAdmins.removeAll(missingAdmins);
    database.updateAdmins(id, new LinkedList<>(unionAdmins));

    builder.clearAdmins();

    for (RecipientId admin : addedAdmins) {
      Recipient recipient = Recipient.resolved(admin);

      if (recipient.getE164().isPresent()) {
        builder.addAdminsE164(recipient.getE164().get());
      }

      builder.addAdmins(createMember(RecipientUtil.toSignalServiceAddress(context, recipient)));
    }
    ...
    if (missingMembers.size() > 0) {
      for (RecipientId missingMember : missingMembers) {
        if (missingMember.equals(Recipient.self().getId())) {
                                       ...
                                       boolean  outgoing)
  {
    ...
    if (members.contains(Recipient.externalPush(context, content.getSender()).getId())) {
      ...
      database.removeAdmin(id, Recipient.externalPush(context, content.getSender()).getId());
  ...
  private static GroupContext.Builder createGroupContext(SignalServiceGroup group) {
    ...
    if (group.getAdmins().isPresent()) {
      builder.addAllAdminsE164(Stream.of(group.getAdmins().get())
              .filter(a -> a.getNumber().isPresent())
              .map(a -> a.getNumber().get())
              .toList());
      builder.addAllAdmins(Stream.of(group.getAdmins().get())
              .map(GroupMessageProcessor::createMember)
              .toList());
    }
