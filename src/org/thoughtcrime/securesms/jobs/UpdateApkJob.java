...
public class UpdateApkJob extends BaseJob {
  ...
  private void handleDownloadStart(String uri, String versionName, byte[] digest) {
    ...
    downloadRequest.setTitle("Downloading update");
    downloadRequest.setDescription("Downloading " + versionName);
    downloadRequest.setVisibleInDownloadsUi(false);
    downloadRequest.setDestinationInExternalFilesDir(context, null, BuildConfig.SIGNAL_FILENAME_PREFIX + "-update.apk");
  ...
  private static void clearPreviousDownloads(@NonNull Context context) {
    ...
    for (File file : directory.listFiles()) {
      if (file.getName().startsWith(BuildConfig.SIGNAL_FILENAME_PREFIX + "-update")) {
        if (file.delete()) {
