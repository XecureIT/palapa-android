...
public class PushGroupSendJob extends PushSendJob {
  ...
  private static final String KEY_X_MEMBERS = "x_members";

  private long        messageId;
  private RecipientId filterRecipient;
  private List<RecipientId> xMembers;

  public PushGroupSendJob(long messageId, @NonNull RecipientId destination, @Nullable RecipientId filterRecipient, List<RecipientId> xMembers) {
    this(new Job.Parameters.Builder()
                           ...
                           .build(),
         messageId, filterRecipient, xMembers);

  }

  private PushGroupSendJob(@NonNull Job.Parameters parameters, long messageId, @Nullable RecipientId filterRecipient, List<RecipientId> xMembers) {
    super(parameters);
                             ...
                             @Nullable List<RecipientId> xMembers)
  {
    try {
      ...
      compressAndUploadAttachment.then(new PushGroupSendJob(messageId, destination, filterAddress, xMembers))
                                 .enqueue();
  ...
  public @NonNull Data serialize() {
    return new Data.Builder().putLong(KEY_MESSAGE_ID, messageId)
                             ...
                             .putString(KEY_X_MEMBERS, xMembers != null && xMembers.size() > 0 ? RecipientId.toSerializedList(xMembers) : null)
                             .build();
  }
      ...
      throws IOException, MmsException, NoSuchMessageException,  RetryLaterException
  {
    ...
    try {
      ...
      List<RecipientId> tMembers = null;
      ...
      if      (filterRecipient != null)            target = Collections.singletonList(Recipient.resolved(filterRecipient).getId());
        ...
        target = getGroupMessageRecipients(message.getRecipient().requireGroupId(), messageId);
        tMembers = xMembers;
      }

      List<SendMessageResult>   results                  = deliver(message, target, tMembers);
  ...
  private List<SendMessageResult> deliver(OutgoingMediaMessage message, @NonNull List<RecipientId> destinations, List<RecipientId> xMembers)
    ...
    if (xMembers != null && xMembers.size() > 0) destinations     = xMembers;
    ...
    if (message.isGroup()) {
      ...
      List<SignalServiceAddress> members          = Stream.of(groupContext.getMembersList())
                                                          ...
                                                          .toList();
      List<SignalServiceAddress> admins           = Stream.of(groupContext.getAdminsList())
                                                          .map(m -> new SignalServiceAddress(UuidUtil.parseOrNull(m.getUuid()), m.getE164()))
                                                          .toList();
      SignalServiceGroup         group            = new SignalServiceGroup(type, GroupUtil.getDecodedId(groupId), groupContext.getName(), members, avatar, groupContext.getOwner(), admins);
  ...
  public static class Factory implements Job.Factory<PushGroupSendJob> {
    ...
    public @NonNull PushGroupSendJob create(@NonNull Parameters parameters, @NonNull org.thoughtcrime.securesms.jobmanager.Data data) {
      ...
      String str = data.getString(KEY_X_MEMBERS);
      List<RecipientId> xMembers = str != null ? RecipientId.fromSerializedList(str) : null;

      return new PushGroupSendJob(parameters, data.getLong(KEY_MESSAGE_ID), filter, xMembers);
    }
  }
}
