...
public class SignalServiceCipher {
  ...
  private SignalServiceGroup createGroupInfo(DataMessage content) throws ProtocolInvalidMessageException {
    ...
    if (content.getGroup().getType() != DELIVER) {
      ...
      String                         owner   = null;
      List<SignalServiceAddress>     admins  = null;
      ...
      if (content.getGroup().getAdminsCount() > 0) {
        admins = new ArrayList<>(content.getGroup().getAdminsCount());

        for (SignalServiceProtos.GroupContext.Member member : content.getGroup().getAdminsList()) {
          if (SignalServiceAddress.isValidAddress(member.getUuid(), member.getE164())) {
            admins.add(new SignalServiceAddress(UuidUtil.parseOrNull(member.getUuid()), member.getE164()));
          } else {
            throw new ProtocolInvalidMessageException(new InvalidMessageException("GroupContext.Member had no address!"), null, 0);
          }
        }
      } else if (content.getGroup().getAdminsE164Count() > 0) {
        admins = new ArrayList<>(content.getGroup().getAdminsE164Count());

        for (String member : content.getGroup().getAdminsE164List()) {
          admins.add(new SignalServiceAddress(null, member));
        }
      }

      if (content.getGroup().hasOwner()) {
        owner = content.getGroup().getOwner();
      }
      ...
      return new SignalServiceGroup(type, content.getGroup().getId().toByteArray(), name, members, avatar, owner, admins);
    }
